...
@Composable private fun WidgetContent(title:String,items:List<FamilyItem>,kind:String,configured:Boolean){Column(GlanceModifier.fillMaxSize().background(ColorProvider(0xFF1B241F)).padding(14.dp)){Row(GlanceModifier.fillMaxWidth(),verticalAlignment=Alignment.CenterVertically){Text(title,style=TextStyle(color=ColorProvider(0xFFB9F34A),fontWeight=FontWeight.Bold,fontSize=17.sp),modifier=GlanceModifier.defaultWeight());Text("↻",style=TextStyle(color=ColorProvider(0xFFA6B0A8),fontSize=18.sp),modifier=GlanceModifier.clickable(actionRunCallback<RefreshAction>()))};Spacer(GlanceModifier.height(8.dp));if(!configured)Text("App öffnen und Gerätezugang einrichten",style=TextStyle(color=ColorProvider(0xFFA6B0A8)),modifier=GlanceModifier.clickable(actionStartActivity<MainActivity>()))else if(items.isEmpty())Text("Alles erledigt",style=TextStyle(color=ColorProvider(0xFFA6B0A8)))else items.take(4).forEach{item->Text("□  ${item.title}",maxLines=1,style=TextStyle(color=ColorProvider(0xFFF3F6F1),fontSize=14.sp),modifier=GlanceModifier.fillMaxWidth().padding(vertical=5.dp).clickable(actionRunCallback<CompleteAction>(actionParametersOf(ItemId to item.id,ItemType to kind))))}}}
