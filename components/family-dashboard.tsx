...
export function FamilyDashboard() {
  ...
  return <div className="min-h-screen bg-background text-foreground"><div className="app-shell">
    ...
    <main className="main-content"><header className="topbar"><button className="menu-button" onClick={() => setMenuOpen(true)} aria-label="Menü öffnen"><Menu /></button><div><p>MITTWOCH, 2. SEPTEMBER</p><h1>Guten Morgen, Julia <span>👋</span></h1></div><div className="top-actions"><label className="search"><Search /><input placeholder="Suchen …" aria-label="Suchen" /></label><Button className="quick-add"><Plus /> Neu hinzufügen</Button><button className="profile-mini" aria-label="Profil"><CircleUserRound /></button></div></header>
      ...
      {active !== 'Übersicht' && <section className="module-notice"><Sparkles /><div><strong>{active}</strong><p>Das Modul ist vorbereitet und wird auf dieser Oberfläche weiter ausgebaut.</p></div><Button onClick={() => setActive('Übersicht')}>Zur Übersicht</Button></section>}

      <section className="stats-row" aria-label="Tagesübersicht"><article><span className="stat-icon lime"><CalendarDays /></span><div><strong>3</strong><small>Termine heute</small></div><em>Alle im Blick</em></article><article><span className="stat-icon orange"><ShoppingBasket /></span><div><strong>{openItems}</strong><small>Offene Einkäufe</small></div><em>Liste teilen</em></article><article><span className="stat-icon blue"><ClipboardCheck /></span><div><strong>2</strong><small>Aufgaben fällig</small></div><em>+20 Punkte</em></article><article><span className="stat-icon violet"><CookingPot /></span><div><strong>12</strong><small>Lieblingsrezepte</small></div><em>2 neu</em></article></section>
